...
func sprintPositionDiff(pPosSys *PositionSystem, b1 PosLayerT, b2 PosLayerT, pRecord *DifferenceRecord) string {
	...
	for file := 9; file > -1; file -= 1 {
		if !pPosSys.PPosition[b1].IsEmptySq(Square(file*10)) || !pPosSys.PPosition[b2].IsEmptySq(Square(file*10)) {
			zeroRanks[10-file] = fmt.Sprintf("%2s%2s", l10.ToCodeOfPc(pPosSys.PPosition[b1].Board[file*10]), l10.ToCodeOfPc(pPosSys.PPosition[b2].Board[file*10]))
		}
	}
	...
	for rank := Square(1); rank < 10; rank += 1 {
		if !pPosSys.PPosition[b1].IsEmptySq(rank) || !pPosSys.PPosition[b2].IsEmptySq(rank) {
			zeroFiles[rank-1] = fmt.Sprintf("%2s%2s", l10.ToCodeOfPc(pPosSys.PPosition[b1].Board[rank]), l10.ToCodeOfPc(pPosSys.PPosition[b2].Board[rank]))
		}
	}
	...
	for file := 9; file > 0; file-- {
		buf.WriteString(fmt.Sprintf("|%2s%2s", l10.ToCodeOfPc(pPosSys.PPosition[b1].Board[file*10+rank]), l10.ToCodeOfPc(pPosSys.PPosition[b2].Board[file*10+rank])))
	}
	...
	for rank := Square(1); rank < 10; rank += 1 {
		for file := Square(9); file > 0; file -= 1 {
			...
			if piece != l10.PIECE_EMPTY {
				...
				pieceString := l10.ToCodeOfPc(piece)
...
func sprintRecord(pRecord *DifferenceRecord) string {
	...
	for i := 0; i < pRecord.OffsetMovesIndex; i += 1 {
		...
		record_text = append(record_text, l10.ToCodeOfPc(pRecord.CapturedList[i])...)
		record_text = append(record_text, '\n')
	}
